...
void os::Amigaos::print_load_average(outputStream* st) {
...
void os::Amigaos::print_uptime_info(outputStream* st) {
  int bootsec = -1;
...
void os::Amigaos::print_rlimit_info(outputStream* st) {
...
void os::Amigaos::print_uname_info(outputStream* st) {
  // kernel
  st->print("uname: ");
  struct utsname name;
  uname(&name);
...
void os::Amigaos::print_umask(outputStream* st, mode_t umsk) {
...
void os::Amigaos::print_user_info(outputStream* st) {
...
const char* os::Amigaos::get_signal_name(int sig, char* out, size_t outlen) {
...
int os::Amigaos::get_signal_number(const char* signal_name) {
  char tmp[30];
...
int os::get_signal_number(const char* signal_name) {
  return os::Amigaos::get_signal_number(signal_name);
}

// Returns true if signal number is valid.
bool os::Amigaos::is_valid_signal(int sig) {
...
bool os::Amigaos::is_sig_ignored(int sig) {
...
const char* os::exception_name(int sig, char* buf, size_t size) {
  if (!os::Amigaos::is_valid_signal(sig)) {
    return NULL;
  }
  const char* const name = os::Amigaos::get_signal_name(sig, buf, size);
...
const char* os::Amigaos::describe_signal_set_short(const sigset_t* set, char* buffer, size_t buf_size) {
...
void os::Amigaos::print_signal_set_short(outputStream* st, const sigset_t* set) {
  char buf[NUM_IMPORTANT_SIGS + 1];
  os::Amigaos::describe_signal_set_short(set, buf, sizeof(buf));
  st->print("%s", buf);
}

// Writes one-line description of a combination of sigaction.sa_flags into a user
// provided buffer. Returns that buffer.
const char* os::Amigaos::describe_sa_flags(int flags, char* buffer, size_t size) {
  char* p = buffer;
...
void os::Amigaos::print_sa_flags(outputStream* st, int flags) {
  char buffer[0x100];
  os::Amigaos::describe_sa_flags(flags, buffer, sizeof(buffer));
  st->print("%s", buffer);
}

// Helper function for os::Amigaos::print_siginfo_...():
...
void os::print_siginfo(outputStream* os, const void* si0) {
  ...
  os->print(" si_signo: %d (%s)", sig, os::Amigaos::get_signal_name(sig, buf, sizeof(buf)));
...
int os::Amigaos::unblock_thread_signal_mask(const sigset_t *set) {
  return pthread_sigmask(SIG_UNBLOCK, set, NULL);
}

address os::Amigaos::ucontext_get_pc(const ucontext_t* ctx) {
#if defined(AIX)
...
void os::Amigaos::ucontext_set_pc(ucontext_t* ctx, address pc) {
#if defined(AIX)
...
char* os::Amigaos::describe_pthread_attr(char* buf, size_t buflen, const pthread_attr_t* attr) {
  size_t stack_size = 0;
  size_t guard_size = 0;
  int detachstate = 0;
...
char* os::Amigaos::realpath(const char* filename, char* outbuf, size_t outbuflen) {

  if (filename == NULL || outbuf == NULL || outbuflen < 1) {
    assert(false, "os::Amigaos::realpath: invalid arguments.");
    errno = EINVAL;
    return NULL;
  }
...
jint os::Amigaos::set_minimum_stack_sizes() {
...
size_t os::Amigaos::get_initial_stack_size(ThreadType thr_type, size_t req_stack_size) {
  size_t stack_size;
  if (req_stack_size == 0) {
...
bool os::Amigaos::is_root(uid_t uid){
    return ROOT_UID == uid;
}

bool os::Amigaos::matches_effective_uid_or_root(uid_t uid) {
    return is_root(uid) || geteuid() == uid;
}

bool os::Amigaos::matches_effective_uid_and_gid_or_root(uid_t uid, gid_t gid) {
...
struct sigaction* os::Amigaos::get_preinstalled_handler(int sig) {
...
void os::Amigaos::save_preinstalled_handler(int sig, struct sigaction& oldAct) {
...
int (*os::Amigaos::_clock_gettime)(clockid_t, struct timespec *) = NULL;
int (*os::Amigaos::_clock_getres)(clockid_t, struct timespec *) = NULL;

bool os::Amigaos::_supports_monotonic_clock = false;
...
static void to_abstime(timespec* abstime, jlong timeout,
  ...
  if (!isAbsolute && os::Amigaos::supports_monotonic_clock()) {
    if (!_use_clock_monotonic_condattr || isRealtime) {
      clock = CLOCK_REALTIME;
    }
    struct timespec now;
    int status = os::Amigaos::clock_gettime(clock, &now);
...
void os::Amigaos::to_RTC_abstime(timespec* abstime, int64_t millis) {
